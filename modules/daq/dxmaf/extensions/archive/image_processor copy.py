...
class ImageProcessor(DataSubscriber):
    ...
    def process(self, channel: str, data, sequence_id: int, timestamp: float) -> None:
        ...
        #logging.info(acquisition_signal)
        ...
        if acquisition_signal == 'ACQUIRING':
            ...
            try:
                ...
                logging.error('Not able to fit Gaussian curve to the intensity plot')
                return
            
            
            if (cols_popt[0] > 10) and (rows_popt[0] > 10):
                if self.roi == None: # ROI is only calculated at the first iteration
                    ...
                    if beamsize_y > 100:
                        beamsize_y = 40
                        sigma = 3
                    self.roi = [com_y-beamsize_y*self.sigma, com_y+beamsize_y*self.sigma, com_x-beamsize_x*self.sigma, com_x+beamsize_x*self.sigma]
                #logging.info('ROI is calculated as: %s', self.roi)
                ...
                try:
                    ...
                    logging.error('Not able to fit Gaussian curve to the cropped intensity plot')
                ...
                #logging.info('Parameters for: %d, %f, %f, %f, %f, %f, %f, %f, %f', sequence_id, com_x, com_y, beamsize_x, beamsize_y, skewness_x, skewness_y, kurtosis_x, kurtosis_y)
            
                if doocs_write == 1:
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/COM_X_MEASUREMENT',com_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/COM_Y_MEASUREMENT',com_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/BEAMSIZE_X_MEASUREMENT',beamsize_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/BEAMSIZE_Y_MEASUREMENT',beamsize_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/SKEWNESS_X_MEASUREMENT',skewness_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/SKEWNESS_Y_MEASUREMENT',skewness_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/KURTOSIS_X_MEASUREMENT',kurtosis_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/KURTOSIS_Y_MEASUREMENT',kurtosis_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/FIT_ERROR_X',fit_error_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/FIT_ERROR_Y',fit_error_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/FIT_ERROR_X_SQUARED',rmse_x)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/FIT_ERROR_Y_SQUARED',rmse_y)
                    pydoocs.write('XFEL.UTIL/DYNPROP/BEAM_PREDICT.'+self.SASE+'/MAX_INTENSITY_ON_SCREEN',max_intensity)
            else:
